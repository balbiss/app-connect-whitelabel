...
export const TransferNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  return (
    <div
      className={`px-3 py-2 shadow-lg rounded-lg border-2 min-w-[200px] max-w-[250px] bg-gradient-to-br from-blue-600/20 to-blue-800/20 relative ${
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <div className="font-semibold text-xs text-white">Transferir para Humano</div>
      </div>

      <div className="space-y-1.5">
        <div>
          <Label className="text-[10px] text-gray-300">Mensagem para o Lead</Label>
          <Textarea
            value={data.message || "Entendido! Vou transferir você para um atendente humano. Aguarde um momento..."}
            onChange={(e) => updateData("message", e.target.value)}
            placeholder="Mensagem de confirmação..."
            className="mt-0.5 text-[10px] bg-black/30 border-blue-500/30 text-white h-16 resize-none"
            rows={2}
          />
        </div>
      </div>
