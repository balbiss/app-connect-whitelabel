  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <div className="font-semibold text-xs text-white">Aguardar</div>
      </div>

      <div className="space-y-1.5">
        <div>
          <Label className="text-[10px] text-gray-300">Timeout (seg)</Label>
          <Input
            type="number"
            value={data.timeout || 300}
            onChange={(e) => handleTimeoutChange(parseInt(e.target.value) || 300)}
            className="mt-0.5 text-[10px] bg-black/30 border-blue-500/30 text-white h-6"
            min={0}
          />
        </div>
